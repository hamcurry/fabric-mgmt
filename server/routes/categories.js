...
// 获取完整分类树（cat1 + cat2）
router.get('/', (req, res) => {
  const cat1List = db.prepare('SELECT * FROM fabric_cat1 ORDER BY sort, id').all()
  const cat2List = db.prepare('SELECT * FROM fabric_cat2 ORDER BY sort, id').all()
...
// 新增一级类目
router.post('/cat1', (req, res) => {
  const { name } = req.body
  if (!name) return res.status(400).json({ error: '类目名不能为空' })
  try {
    const r = db.prepare('INSERT INTO fabric_cat1(name) VALUES(?)').run(name)
...
// 新增二级类目
router.post('/cat2', (req, res) => {
  const { cat1_id, name } = req.body
  if (!cat1_id || !name) return res.status(400).json({ error: '参数不完整' })
  const r = db.prepare('INSERT INTO fabric_cat2(cat1_id, name) VALUES(?,?)').run(cat1_id, name)
  res.json({ id: r.lastInsertRowid })
})

// 编辑二级类目
router.put('/cat2/:id', (req, res) => {
  const { name } = req.body
